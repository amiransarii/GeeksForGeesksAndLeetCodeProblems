 ...
 *lds[i] stores the length of the longest Decreasing subsequence starting from arr[i]. Finally,
 * we need to return the max value of lis[i] + lds[i] – 1 where i is from 0 to n-1
 */
public class TestLongestBitonicSubsequence {
    ...
    */
	...
	static int lbs(int[] arr, int n) {
		...
		/**
		 * computer LIS values from left to right
		 */
		for(i = 1; i <n ; i++)
			for(j = 0; j < i; j++)
		...
		int max = lis[0] + lds[0] -1;
		  ...
		  for (i = 1; i < n; i++) 
	            if (lis[i] + lds[i] - 1 > max) 
	                max = lis[i] + lds[i] - 1;  // -1 as arr[i] counted twice
